...
LinkingServiceSensor.prototype._parseOriginalData = function (buf, sensor_type) {
	...
	if (sensor_type === 0x03) { // Battery
		return {
			chargeRequired: (n & 0b1000100000000000) ? true : false,
		...
		let v = buf.readFloatLE(0);
		return {
			pressure: v
		};
	} else if (sensor_type === 0x07) { // Opening and closing
		let open = (n & 0b1000100000000000) ? true : false;
		...
		let response = (n & 0b1000100000000000) ? true : false;
