			...
			p.connect((error) => {
				if(error) {
					...
					this._init().then(() => {
						...
						return this.write('CONFIRM_NOTIFY_CATEGORY');
					}).then((res) => {
						...
						return this.write('GET_SETTING_INFORMATION');
					}).then((res) => {
						this.info['settings'] = {};
						...
						return this._writeGetSettingInformation('LEDColorName');
					}).then((res) => {
						if(res) {
							this.info['settings']['LED']['colors'] = res['data']['settingNameData'];
						}
						return this._writeGetSettingInformation('LEDPatternName');
					}).then((res) => {
						if(res) {
							this.info['settings']['LED']['patterns'] = res['data']['settingNameData'];
						}
						return this._writeGetSettingInformation('VibrationPatternName');
					}).then((res) => {
						if(res) {
							this.info['settings']['Vibration']['patterns'] = res['data']['settingNameData'];
						}
						return this._writeGetSettingInformation('BeepPatternName');
					}).then((res) => {
						if(res) {
...
LinkingDevice.prototype._writeGetSettingInformation = function(name) {
	let promise = new Promise((resolve, reject) => {
