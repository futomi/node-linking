...
/* ------------------------------------------------------------------
* Constructor: LinkingServiceSetting()
* ---------------------------------------------------------------- */
const LinkingServiceSetting = function() {
...
* ---------------------------------------------------------------- */
LinkingServiceSetting.prototype.setDeviceInfo = function(info) {
	this._device = info;
};
...
* ---------------------------------------------------------------- */
LinkingServiceSetting.prototype.parsePayload = function(pnum, buf) {
	let offset = 0;
	let parameters = [];
	try {
...
LinkingServiceSetting.prototype._parseParameter = function(pid, buf) {
	let parsed = null;
	if(pid === 0x00) {
...
LinkingServiceSetting.prototype._parseResultCode = function(buf) {
...
LinkingServiceSetting.prototype._parseCancel = function(buf) {
	let code = buf.readUInt8(0);
	let text = '';
	if(code === 0x00) {
		text = 'User cancel';
...
LinkingServiceSetting.prototype._parseSettingNameType = function(buf) {
	let code = buf.readUInt8(0);
	let text = '';
	if(code === 0x00) {
		text = 'LEDColorName';
...
LinkingServiceSetting.prototype._parseAppName = function(buf) {
	return {
		name    : 'AppName',
...
LinkingServiceSetting.prototype._parseFileVer = function(buf) {
	let list = [];
...
LinkingServiceSetting.prototype._parseFileSize = function(buf) {
	return {
		name     : 'FileSize',
...
LinkingServiceSetting.prototype._parseInstallConfirmStatus = function(buf) {
	let code = buf.readUInt8(0);
	let text = '';
	if(code === 0x00) {
...
LinkingServiceSetting.prototype._parseSettingInformationRequest = function(buf) {
	let code = buf.readUInt8(0);
	let text = '';
	if(code === 0x00) {
		text = 'SETTING';
	...
	let settings = [];
	while(true) {
		let setting_code = buf.readUInt8(offset);
...
LinkingServiceSetting.prototype._parseSettingInformationData = function(buf) {
	let offset = 0;
	let settings = [];
	while(true) {
		let setting_code = buf.readUInt8(offset);
...
LinkingServiceSetting.prototype._parseSettingNameData = function(buf) {
	let list = [];
	let offset = 0;
	while(true) {
		let len = buf.readUInt8(offset++);
		let name = buf.slice(offset, offset + len - 1).toString('utf-8');
		offset += len;
		list.push(name);
		if(offset >= buf.length) {
			break;
...
* ---------------------------------------------------------------- */
LinkingServiceSetting.prototype.createRequest = function(message_name, params) {
...
LinkingServiceSetting.prototype._createPayload = function(message_name, params) {
...
LinkingServiceSetting.prototype._createPropertyBlockBuffer = function(pid, val_buf) {
...
LinkingServiceSetting.prototype._createPayloadGetAppVersion = function(params) {
	let pnum = 0;
	let app_name = null;
	...
	// AppName
	if(app_name !== null) {
		let val_buf = Buffer.from(app_name);
...
LinkingServiceSetting.prototype._createPayloadConfirmInstallApp = function(params) {
	let pnum = 0;
	let app_name = null;
	if(('AppName' in params) && typeof(params['AppName']) === 'string') {
		app_name = params['AppName'];
	...
	// FileVer
	if(file_ver !== null) {
		let val_buf = Buffer.from(file_ver);
...
LinkingServiceSetting.prototype._createPayloadGetSettingInformation = function(params) {
	let pnum_buf = Buffer.from([0]);
	return pnum_buf;
};

LinkingServiceSetting.prototype._createPayloadGetSettingName = function(params) {
	let pnum = 0;
	let type = null;
	if('SettingNameType' in params) {
		...
		if(typeof(type) === 'number') {
			let setting_name = '';
			for(let name in this._SETTING_NAME_TYPE_MAP) {
...
LinkingServiceSetting.prototype._createPayloadSelectSettingInformation = function(params) {
	let pnum = 0;
	let code = null;
	if(('SettingInformationRequest' in params) && typeof(params['SettingInformationRequest']) === 'object') {
		let req = params['SettingInformationRequest'];
	...
	if('SettingInformationData' in params) {
		if(Array.isArray(params['SettingInformationData'])) {
			...
			for(let i=0; i<params['SettingInformationData'].length; i++) {
				let s = params['SettingInformationData'][i];
				let scode = null;
				let sname = '';
				...
				let pm = this._device['settings'][sname]['patternMax'];
	...
	if(settings !== null) {
		...
		let val_buf = Buffer.concat(val_buf_list);
...
module.exports = LinkingServiceSetting;